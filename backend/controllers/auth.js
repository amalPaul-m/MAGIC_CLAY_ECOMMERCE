...
export const login = async (req, res, next) => {
    ...
    try {
        ...
        const isVerify = userData.otp;
        if(isVerify!==null) {
            return res.status(HTTP_STATUS.NOT_FOUND).json({ message: 'Email Verification required' });
        }
...
export const checkAuthSession = (req, res) => {
    ...
    if (!req.session || !req.session.user) {
        return res.status(HTTP_STATUS.UNAUTHORIZED).json({ message: "Not logged in" });
    }
